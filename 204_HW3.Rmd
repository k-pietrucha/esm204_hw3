...
# Katie question - does it make intuitive sense that the new p is lower? shouldn't it be higher with the tax?
...
# Katie note - tried these but they must be wrong, don't make intuitive sense :-(

tax_cs_sq_high <- 0.5*q_star*((tax_agg_y_int/100) - (p_star/100))
# High consumer benefit $50,827.35
tax_cs_sq_low <- 0.5*q_star*((tax_low_y_int/100) - (p_star/100))
# Low consumer benefit $30,225.91

tax_cs_sq_total <- tax_cs_sq_high + tax_cs_sq_low
# Consumer benefit $81,053.25

tax_ps_sq <- 0.5*q_star*(p_star/100)
# Producer benefit $23,316.15
tax_env_cost_sq <- (MEC*q_star)/100
