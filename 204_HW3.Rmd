...
cost_table <- tibble(SCC) %>% 
  ...
  mutate(cs_high_yesrevenue = cs_high_norevenue +high_cs_revenue) %>% 
  mutate(cs_low_norevenue = cs(p_star_tax, model_demand_l) - (MEC * q_star)) %>% 
  mutate(cs_low_yesrevenue = cs_low_norevenue + low_cs_revenue ) %>% 
  mutate(ps = ps(p_star)) %>% 
  mutate(tec = (MEC * q_star))

final_cost_table = subset(cost_table, 
                          select = c(SCC,cs_high_yesrevenue,cs_low_yesrevenue,ps,tec) )
...
kable(final_cost_table, caption = "Table 1: Welfare levels for High and Low income consumers and Electricity Producers based on varying social costs of carbon.", 
      col.names = c("Social Cost of Carbon (USD)", "High Income Welfare (USD)", "Low Income Welfare (USD)", "Electricity Producer Welfare (USD)")) %>% 
  kable_styling(
