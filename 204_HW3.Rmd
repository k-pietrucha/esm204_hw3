...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE, scipen = 9999999)
...
demand_high_taxed <- function(p){
  q <-demand_tax(p, model_demand_h)
  return(q)
}

demand_low_taxed <- function(p){
  q <- demand_tax(p, model_demand_l)
  return(q)
}
...
tax_cs_high <- 0.5*(demand_high_taxed(p_star))*((tax_agg_y_int/100) - ((p_star/100) + MEC/100))
# High consumer benefit $35,845.77

tax_cs_low <- 0.5*(demand_low_taxed(p_star))*((tax_low_y_int/100) - ((p_star/100) + MEC/100)) - ((MEC/100)*q_star)
